...
void main()
{
    ...
    vec3 diffuse = lightColor.rgb * NdotL * 0.4; // Reduced from 1.0 to 0.7
    ...
    vec3 specular = lightColor.rgb * spec * 0.4; // Reduced from 0.4 to 0.2
    ...
    // Softer shadow sampling with more samples and larger kernel
    int shadowCounter = 0;
    ...
    vec3 lighting = ambient.rgb * 0.8 + (1.0 - shadow) * (diffuse + specular);
